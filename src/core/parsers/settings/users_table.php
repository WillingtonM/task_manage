        ...
        <?php if (is_array($usr_qry) || is_object($usr_qry)) : ?>
            ...
            <?php foreach ($usr_qry as $key => $usr) : ?>
                ...
                <tr>
                    ...
                    <td class="">
                        <button type="button" class="btn btn-warning float-end me-2" name="button" onclick="requestModal(post_modal[22], post_modal[22],  {'user': <?= $usr['user_id'] ?>})" style="padding: 5px 15px;" <?= ((!$is_admin) ? 'disabled' : '') ?>> <span><i class="fa-solid fa-list-check me-2"></i> Tasks </span> </button>
