        ...
        <div class="col-12">
            ...
            <?php else : ?>
                <div class="row shadow-sm px-0 mb-3 bg-white rounded" style="border-radius: 15px !important;">
                    ...
                    if (HOST_IS_LIVE) {
                        $usr_assign     = (!empty($user)) ? ' AND assigned_to = ' . $user_id : ''; 
                        $rgst_sql       = "SELECT * FROM tasks WHERE task_status = 1 AND company_id = ? " . $usr_assign . " ORDER BY task_date_updated DESC LIMIT $sql_pg_strt, $intval";
                        $rgst_dta       = [$company_id];
